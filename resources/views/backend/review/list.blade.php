...
@section('content')
    ...
    <form action="{{ route('backend.config.reviews.postDeleteMuti') }}" method="POST">
        <input type="hidden" name="_token" value="{!! csrf_token() !!}">
        <div class="btnAdd">
            <a href="{{ route('backend.config.reviews.getAdd') }}">
            ...
            <tbody>
                 @foreach($data as $key => $item)
                    <tr>
                        ...
                        <td>{!! $key + 1 !!}</td>
                        <td>
                            <img src="{{ asset( 'uploads/config/reviews/'.$item['image'] ) }}" class="img-thumbnail">
                        </td>
                        <td>
                            {{ $item->name }}
                        </td>
                        ...
                        <td>
                            <div>
                                <a href="{{ route('backend.config.reviews.getEdit', $item->id) }}" title="Sửa">
                                    <i class="fa fa-pencil fa-fw"></i> Sửa
                                </a> &nbsp; &nbsp; &nbsp;
                                <a class="text-danger" href="{{ route('backend.config.reviews.getDelete', $item->id) }}" 
